...
class GaussianDiff:
    ...
    def __init__(self, dt, A_max, F_max, Sc=1.0, eps=1.0, mu=1.0):
    ...
    def getE(self, m, q):
        ...
        w = np.sqrt(np.log(self.A_max)) / (np.pi * self.F_max)
        d = w * np.sqrt(np.log(self.A_max))
        
        return np.exp(-(((q - m * np.sqrt(self.eps * self.mu) / self.Sc) - d/dt) / (w/dt)) ** 2)
...
if __name__ == '__main__':
    ...
    layer_x_2=layer_x+0.02
    layer_x_2_DX=int(np.ceil(layer_x_2 / d))
    layer_x_3=layer_x+0.02+0.04
    ...
    probesPos = [int(np.ceil(0.7 / d)),int(np.ceil(0.2 / d))]
    
    probes = [tools.Probe(pos, maxTime) for pos in probesPos]

    # Параметры среды
    eps = np.ones(maxSize)
    eps[layer_x_DX:layer_x_2_DX] = 2
    eps[layer_x_2_DX:layer_x_3_DX] = 6.8
    ...
    source = GaussianDiff(dt,1000,20e9)
    ...
    start_index_pad=sourcePos+layer_x_DX*2
    ...
    #if start_index_pad < len(Ezspectrumpad):
        #Ezspectrumpad[start_index_pad:] = 1e-18
        

    spectrumpad = np.abs(fft(Ezspectrumpad))+np.abs(fft(Ezspectrumotr))
